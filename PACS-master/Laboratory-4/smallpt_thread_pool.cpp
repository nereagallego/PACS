...
int main(int argc, char *argv[]){
    ...
    if (argc == 1) {
        ...
        thread_pool *pool = new thread_pool({h});

        // launch the tasks by rows
        for (size_t i=0; i<h; i++) {
            pool->submit([=] {
                render(w, h, samps, cam, cx, cy, c_ptr, Region(0, w, i, i+1));
            });
        }
        ...
        // create a thread pool
        thread_pool *pool2 = new thread_pool({w});

        // launch the tasks by columns
        for (size_t i=0; i<w; i++) {
            pool2->submit([=] {
                render(w, h, samps, cam, cx, cy, c_ptr, Region(i, i+1, 0, h));
            });
        }
